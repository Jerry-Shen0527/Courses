	...
	if (dimension == 0)
	{
		for (M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); fiter++)
		{
			...
			for (M::FaceHalfedgeIterator fhiter(pf); !fhiter.end(); fhiter++)
			{
				...
				M::CVertex* end = static_cast<M::CVertex*> (ph->vertex());
				M::CVertex* beg = static_cast<M::CVertex*> (ph->source());

				ph->form() = end->form() - beg->form();
...
void MeshLib::CHodgeDecomposition::_delta(int dimension)
{
	...
	if (dimension == 2)
	{
		for (M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); fiter++)
		{
			...
			for (M::FaceHalfedgeIterator fhiter(pf); !fhiter.end(); fhiter++)
			{
				...
				auto w = static_cast<M::CEdge*>(ph->edge())->weight();

				auto sym_he = ph->he_sym();
				if (sym_he != nullptr)
				{
					auto sym_face = static_cast<M::CFace*> (ph->he_sym()->face());

					if (sym_face != nullptr)
					{
						ph->form() = -1 / w * (pf->form() - sym_face->form());
					}
				}
				else
				{
					ph->form() = -1 / w * (pf->form() - 0);
				}
			}
		}
		return;
	...
	if (dimension == 1)
	{
		for (M::MeshVertexIterator viter(m_pMesh); !viter.end(); viter++)
		{
			M::CVertex* pv = *viter;
			pv->form() = 0;
			for (M::VertexOutHalfedgeIterator oiter(m_pMesh, pv); !oiter.end(); oiter++)
			{
				auto ptr = *oiter;

				auto e = static_cast<M::CEdge*>  (ptr->edge());
...
void MeshLib::CHodgeDecomposition::_remove_exact_form()
{
	...
	for (M::MeshFaceIterator fiter(pM); !fiter.end(); fiter++)
	{
		...
		{
			M::CHalfEdge* ph = *fhiter;
			//isert your code here, remove d vertex->form() from halfedge->form()

			auto end = static_cast<M::CVertex*> (ph->vertex());
			auto beg = static_cast<M::CVertex*>(ph->source());
	...
	for (M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); ++fiter)
	{
		...
		for (M::FaceHalfedgeIterator fhiter(pf); !fhiter.end(); ++fhiter)
		{
			...
			if (ps != NULL)
			{
				...
				M::CFace* adj = static_cast<M::CFace*> (ps->face());

				auto w = -1 / static_cast<M::CEdge*> (ps->edge())->weight();
				sw += w;
				int wid = adj->idx();
				A_coefficients.emplace_back(fid, wid, w);
			}
			else // boundary face
			{
				auto w = -1 / static_cast<M::CEdge*> (ps->edge())->weight();
...
void MeshLib::CHodgeDecomposition::_remove_coexact_form()
{
	...
	for (M::MeshFaceIterator fiter(pM); !fiter.end(); fiter++)
	{
		...
		for (M::FaceHalfedgeIterator fhiter(pf); !fhiter.end(); fhiter++)
		{
			...
			auto e = static_cast<M::CEdge*> (he->edge());
			...
			he->form() -= -1 / e->weight() * (sym_area - pf->form());

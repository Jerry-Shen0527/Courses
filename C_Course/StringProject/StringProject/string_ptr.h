...
extern char* buffer[SIZE] = { NULL };

int find_pos()
{
	int storage = 1;
	while (buffer[storage] != 0)
	{
		storage++;
		if (storage > SIZE)
		{
...
int string_read()
{
	...
	buffer[storage] = malloc(200 * sizeof(char)); buffer[storage][0] = 0;//置为空串
...
int string_cpy(int index, int index2)
{
	...
	if (!buffer[storage])
	{
		buffer[storage] = malloc(200 * sizeof(char));
	}
	 buffer[storage][0] = 0;//置为空串
	udf_strcpy(0, buffer[index], buffer[storage]);
	printf("Result saved at %d\n", storage);
}

...
namespace link_list
{
	...
	class LinkList
	{
		...
		template<typename TT>
		friend std::ostream& operator<<(std::ostream& os, const LinkList<TT>& list);
	public:
		...
		int find(const T&) const;//-1 marks fail to findlist
	...
	T& LinkList<T>::get_elem(const int index) const
	{
		...
		{
			std::cerr << e.what() << '\n';
			std::cerr << "Try a new index?(Y or n)" << std::endl;
			if (getchar() != 'y')
				exit(0);
			else
			{
				return get_elem(getchar());
			}
		}
	...
	void LinkList<T>::insert(const T & in, int index)
	{
		...
		if (index > number_of_items)
		{
			std::cerr << "input index is larger than the size!" << std::endl;
			return;
		}
	...
	template<typename TT>
	std::ostream& operator<<(std::ostream& os, const LinkList<TT>& list)
	{
		DataItem<TT>* cur = list.head->get_ptr();
		while (cur->get_ptr())
		{
			os << cur->data() << ' ';
			cur = cur->get_ptr();
